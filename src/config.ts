...
export async function loadLinterConfig(
  ...
  return { overrides: parseConfig(found.raw, found.source), source: found.source }
}
...
async function resolveConfigSource(
  ...
  if (existsSync(pkgPath)) {
    ...
    if (value !== undefined) {
      if (!isObject(value)) {
        throw new Error(`"${PACKAGE_JSON_KEY}" in ${pkgPath} must be an object.`)
      }
...
async function readJson(file: string): Promise<RawConfig> {
  ...
  try {
    ...
    throw new Error(`Unable to read config file ${file}: ${(error as Error).message}`)
  }
  try {
...
function parseConfig(raw: RawConfig, source: string): SettingsOverrides {
  ...
  if (raw.classAttributes !== undefined) {
    overrides.classAttributes = parseStringArray(raw.classAttributes, 'classAttributes', source)
  }
  if (raw.classFunctions !== undefined) {
    overrides.classFunctions = parseStringArray(raw.classFunctions, 'classFunctions', source)
  }
  if (raw.includeLanguages !== undefined) {
    overrides.includeLanguages = parseStringRecord(raw.includeLanguages, 'includeLanguages', source)
  }
...
function parseRules(value: unknown, source: string): Partial<Record<RuleName, RuleSeverity>> {
  if (!isObject(value)) {
...
function parseStringRecord(value: unknown, field: string, source: string): Record<string, string> {
  if (!isObject(value) || Object.values(value).some((item) => typeof item !== 'string')) {
    throw new Error(`"${field}" in ${source} must be an object mapping strings to strings.`)
  }
