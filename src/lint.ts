...
import { collectFixEdits, applyTextEdits } from './fix.js'
import { DEFAULT_GLOBS, DEFAULT_IGNORE, languageIdForFile } from './languages.js'
  ...
  try {
    ...
    if (!projectDetected) {
      for (const { filePath } of files) {
        results.push({ filePath, messages: [], errorCount: 0, warningCount: 0 })
      }
    ...
    for (const { filePath } of files) {
      ...
      if (fix !== 'none') {
        const actions = await client.codeActions(filePath, source.text, diagnostics)
